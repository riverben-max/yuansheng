...
## 8. 采集专用浏览器的启动与登录态持久化（2026-05-31 定稿）

> 这是反复踩坑后的最终方案，**改动前务必看完**。涉及 `relaunch_browser_for_debug`。

### 核心模型

软件管理的是一个**独立的"采集专用浏览器"窗口**，跟客户日常用的 360 是**两个分开的窗口**（不同数据目录），互不影响：

- 日常 360：客户平时浏览网页用，默认 profile
- 采集专用浏览器：软件用 `--user-data-dir=%LOCALAPPDATA%\YuanshengDataAssistant\browser-debug-profile` + `--remote-debugging-port=9527` 启动的独立窗口

**判断依据是"调试端口 9527 在不在"（即专用浏览器在不在跑），不是"日常 360 启没启动"。**

### 点「浏览器」按钮的完整流程

```
点「浏览器」→ grab_browser_cookie 先 detect_browser_debug_status(9527)
   │
   ├── 端口没开（专用浏览器没跑）
   │     → 返回 needsSetup=true → 前端弹"重启浏览器"确认
   │     → relaunch_browser_for_debug：
   │          · 端口仍没开 → 用独立 profile + 端口 + 最小参数启动专用浏览器，打开对应平台登录页
   │          · （端口已开 → 复用，不强杀，直接打开页面）
   │     → 客户在专用窗口登录（首次。登录态存进独立 profile）
   │
   └── 端口已开（专用浏览器在跑）
         → 抓 cookie（千牛走第 2 节完整性校验/重抓）
         → 完整则保存 → 采集
```

### 三条铁律（违反就会回到坑里）

1. **必须用独立固定 profile**：360 带调试端口会隔离日常 profile，读不到客户日常登录态——实测用日常 `User Data` 目录启动仍是登录页。独立 profile 登录一次后持久，以后复用免登录。
2. **端口已开就复用、绝不 `taskkill` 强杀**：强杀固定 profile → 360 下次启动弹"崩溃恢复"对话框，阻断自动化。
3. **只用最小启动参数**（`--remote-debugging-port` + `--user-data-dir` + url）：`--disable-session-crashed-bubble` / `--no-first-run` 等会暴露自动化特征，触发淘宝手机验证（commit 3856e14）。subprocess 用列表传参，`User Data` 路径含空格也安全。

### 客户体验

- **首次**：点「浏览器」→ 专用窗口打开登录页 → 登录一次（可能手机验证一次，一次性）→ 再点「浏览器」→ 采集
- **之后**：登录态持久在独立 profile，点「浏览器」直接复用、免登录、直接采集
- 客户日常浏览器开不开都不影响，软件只管自己那个专用窗口
